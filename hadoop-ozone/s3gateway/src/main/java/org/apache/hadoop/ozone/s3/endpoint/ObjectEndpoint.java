  ...
  Response handlePutRequest(ObjectRequestContext context, String keyPath, InputStream body) throws IOException {
    ...
    try {
      ...
      if (isDatastreamEnabled() && !enableEC && length > getDatastreamMinLength()) {
        ...
        try (OzoneOutputStream ozoneOutputStream = openKeyForPut(
            ...
            S3ObjectWriteGuard output = new S3ObjectWriteGuard(ozoneOutputStream, expectedLength, keyPath)) {
          long metadataLatencyNs =
              getMetrics().updatePutKeyMetadataStats(startNanos);
          perf.appendMetaLatencyNanos(metadataLatencyNs);
    ...
    try {
      ...
      if (copyHeader != null) {
        ...
        try (OzoneOutputStream ozoneOutputStream = getClientProtocol()
                .createMultipartKey(volume.getName(), bucketName, key, expectedLength, partNumber, uploadID, isSignedInputStream);
            S3ObjectWriteGuard writeGuard = new S3ObjectWriteGuard(ozoneOutputStream, expectedLength, key)) {
          metadataLatencyNs =
              getMetrics().updatePutKeyMetadataStats(startNanos);
          if (ozoneOutputStream.getDerivedKey() != null) {
            InputStream wrapped = multiDigestInputStream.getWrappedInputStream();
